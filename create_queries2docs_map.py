...
for q in queries_d['parameters']['query']:
    ...
    query_map[q['number']] = {'class': c_l, 'method': m_l,
                              'identifier': i_l, 'comments': o_l}
...
for _id in docs_d:
    ...
    # WTF ... org.eclipse.swt.internal.win32.OS.java <class> is empty
    c = docs_d[_id]['DOC']['text']['class']
    if c is None: c = ''
    ...
    docs_map[_file] = {'class': c_l, 'method': m_l,
                       'identifier': i_l, 'comments': o_l}


# json.dumps(docs_map)

bug_report_sentences = {'class': [], 'method': [],
                        'identifier': [], 'comments': []}
fixed_file_sentences = {'class': [], 'method': [],
                        'identifier': [], 'comments': []}
...
with open('bug_report_sentences.txt', 'w') as outfile:
    json.dump(bug_report_sentences, outfile,
              sort_keys=True, indent=4, ensure_ascii=False)

with open('fixed_file_sentences.txt', 'w') as outfile:
    json.dump(fixed_file_sentences, outfile,
              sort_keys=True, indent=4, ensure_ascii=False)
